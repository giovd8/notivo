...
const registerUser = async (payload: UserCredentialDTO): Promise<UserDTO> => {
  ...
  if (!resp.ok) {
    if (resp.status === 409) throw new ServerError("Username already taken", 409);
    let errorBody = "";
    try { errorBody = await resp.text(); } catch { }
    throw new ServerError(errorBody, resp.status);
  }
  const json = await resp.json();
  const user = json?.data as { id: string; username: string; createdAt: string } | null;
  if (!user?.id)
    throw new ServerError("User id not found", 500);
...
const refreshTokens = async (refreshToken: string): Promise<{ accessToken: string; refreshToken: string }> => {
  if (!refreshToken)  throw new ServerError("Invalid refresh token", 401)
  const session = await findSessionByRefreshToken(refreshToken);
  if (!session) throw new ServerError("Invalid refresh token", 401)
  try {
...
const logoutUser = async (refreshToken: string): Promise<void> => {
  ...
  if (!existing) throw new ServerError("Invalid refresh token", 401)
